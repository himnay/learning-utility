...
### Why verification tolerates clock drift
...
### Verification — `verify(accountName, code)`
...
### QR re-render — `generateQrCodeImage(accountName)`
...
### Persistence — `TotpSeedRepository` and the `totp_seed` table
...
### 1. Generic QR encode/decode — `com.learning.qr`
...
### 2. TOTP enrollment QR — `com.learning.totp`, via `dev.samstevens.totp`
...
### Why it's covered in this README at all
...
### Verification flow — user submits a code, server validates it against the time window
...
#### `GET /qr/generate` — encode arbitrary text as a QR code PNG
...
#### `POST /notifications/send` — send an Apple Push Notification to a device
